...
response=$(curl -s -X PUT $host/api/auth -d '{"email":"a@jwt.com", "password":"canyoureallyguessmypassword"}' -H 'Content-Type: application/json')
token=$(echo $response | jq -r '.token')

# Add users
curl -X POST $host/api/auth -d '{"name":"pizza diner", "email":"d@jwt.com", "password":"thebestdiner"}' -H 'Content-Type: application/json'
curl -X POST $host/api/auth -d '{"name":"pizza franchisee", "email":"f@jwt.com", "password":"thebestfranchisee"}' -H 'Content-Type: application/json'
